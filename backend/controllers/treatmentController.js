...
// ==========================================
// بخش مربوط به جستجوی هوشمند (Auto-complete)
// ==========================================

// @desc    دریافت لیست بیماری‌های موجود بر اساس گونه (بدون تکرار)
// @route   GET /api/v1/calculator/conditions/:species
exports.getConditionsBySpecies = async (req, res) => {
    try {
        const { species } = req.params;

        // استفاده از distinct برای استخراج نام بیماری‌ها بدون تکرار
        const conditions = await TreatmentProtocol.distinct('condition', { species });
        ...
        console.error('خطا در دریافت لیست بیماری‌ها:', error);
        return res.status(500).json({
            success: false,
